...
public class Chord extends java.rmi.server.UnicastRemoteObject implements ChordMessageInterface {
     ...
     *
     */
    private static final long serialVersionUID = 552291129303774191L;
    ...
    public String search(String keyword, long pageGuid) throws RemoteException {
        List<SongValues> results = new ArrayList<>();
        ...
        try {
            ...
            SongValues[] records = gson.fromJson(fileStr, SongValues[].class);
            for (SongValues record : records) {
                if (record.retArtist().retname().toLowerCase().contains(keyword.toLowerCase())
                        || record.retSong().rettitle().toLowerCase().contains(keyword.toLowerCase())) {
                    results.add(record);
                }
            }
        } catch (IOException e) {
            throw (new RemoteException("IO exception in Chord:" + e));
        }
        try {
            fileStream.close();
        } catch (IOException e) {
